...
namespace Ecore.MsSql
{
    public class Entity<T> where T : class, new()
    {
        ...
        protected string GetKey()
        {
            ...
            {
                var keyAttr = (KeyAttribute)q.DeclaringType.GetTypeInfo().GetCustomAttributes<KeyAttribute>(); 
                if (keyAttr != null)
                {
                    return true;
                }
        ...
        protected string GetUniqueKey()
        {
            ...
            string key = minfos.Where(q =>
            {
                var keyAttr = (UniqueAttribute)q.DeclaringType.GetTypeInfo().GetCustomAttributes<UniqueAttribute>(); 
                if (keyAttr != null)
                {
                    return true;
                }
